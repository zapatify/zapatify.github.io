...
Given all the AI development I'm doing and because AI lives in the markdown world, I wanted a markdown editor that behaves like a Mac app should. The market is split between over-engineered tools (Obsidian, Bear, iA Writer) and broken ones — there's an editor in the App Store right now that can't even move its own window. I just wanted something that opens a file, lets me edit it, and gets out of the way.
...
3. **Mac App Store Readiness**: Sandboxed, signed, and shipped with the proper entitlements — no network access required or requested
